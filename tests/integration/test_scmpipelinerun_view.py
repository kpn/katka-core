...
class TestSCMPipelineRunViewSet:
    def test_list(self, client, logged_in_user, my_application, my_scm_pipeline_run, my_scm_release):
        response = client.get("/scm-pipeline-runs/")
        assert response.status_code == 200
        parsed = response.json()
        assert len(parsed) == 1
        assert parsed[0]["commit_hash"] == "4015B57A143AEC5156FD1444A017A32137A3FD0F"
        assert UUID(parsed[0]["application"]) == my_application.public_identifier
    ...
    def test_filtered_list(
        ...
        response = client.get("/scm-pipeline-runs/?application=" + str(my_other_application.public_identifier))
        assert response.status_code == 200
        parsed = response.json()
        assert len(parsed) == 1
        assert parsed[0]["commit_hash"] == "1234567A143AEC5156FD1444A017A3213654321"
        assert UUID(parsed[0]["application"]) == my_other_application.public_identifier
